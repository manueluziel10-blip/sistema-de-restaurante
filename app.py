...
# --- SECCIÓN 2: GESTIÓN EMPLEADOS ---
    ...
    with st.expander("Modificar / Agregar Empleado"):
        ...
        with c_mod:
            n = st.selectbox("Empleado", empleados_df['nombre'].tolist(), key="sel_emp_modificar")
            t = st.selectbox("Nuevo Puesto", list(PUESTOS_CATALOGO.keys()), key="sel_nuevo_puesto")
            s = st.number_input("Sueldo Base ($)", value=300.0, key="num_sueldo_mod")
            if st.button("Actualizar"): actualizar_empleado(n, t, s); st.rerun()
        with c_add:
            n_name = st.text_input("Nombre", key="input_nuevo_nombre")
...
# --- SECCIÓN 3: CORTE Y NÓMINA ---
    ...
    with t_b:
        ...
        df_b = st.data_editor(pd.DataFrame(data), column_config={"Sueldo Base": st.column_config.NumberColumn(format="$%.2f")}, key="editor_bailarinas_tabla")
    ...
    with t_g:
        ...
        df_g = st.data_editor(pd.DataFrame(data_g), column_config={"Sueldo Base": st.column_config.NumberColumn(format="$%.2f")}, key="editor_general_tabla")
...
# --- SECCIÓN 4: DASHBOARD ---
    ...
    st.info("Panel de control y cierre diario consolidado.")