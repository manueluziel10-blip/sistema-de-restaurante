...
from models import (
    ...
    inicializar_usuarios_por_defecto, validar_login,
    verificar_corte_bloqueado, desbloquear_corte_fecha, bloquear_corte_fecha
)

st.set_page_config(layout="wide")

# Inicializar usuarios por defecto al arrancar por si la BD ya existe
...
if st.session_state["usuario_actual"] is None:
    ...
    with st.sidebar.form("form_login"):
        ...
        if btn_login:
            datos_user = validar_login(user_input, pass_input)
            if datos_user:
                st.session_state["usuario_actual"] = datos_user
                st.success(f"¡Bienvenido, {datos_user['username']} ({datos_user['rol']})!")
                st.rerun()
            else:
                st.error("Usuario o contraseña incorrectos.")
    
    st.stop()  # Detener la ejecución hasta que inicie sesión
else:
    user_actual = st.session_state["usuario_actual"]
    st.sidebar.success(f"Sesión activa: **{user_actual['username']}** ({user_actual['rol'].upper()})")
...
# Verificar si la fecha activa está bloqueada
corte_bloqueado = verificar_corte_bloqueado(fecha_activa)

if corte_bloqueado:
    st.sidebar.error(f"🔒 Fecha bloqueada (Cierre cerrado)")
    if user_actual['rol'] == 'admin':
        if st.sidebar.button("🔓 Desbloquear esta fecha (Admin)"):
            desbloquear_corte_fecha(fecha_activa)
            st.sidebar.success("¡Fecha desbloqueada con éxito!")
            st.rerun()
else:
    st.sidebar.success(f"🔓 Fecha abierta para edición")
...
# --- BOTÓN DE REINICIO EN LA BARRA LATERAL (SOLO ADMIN) ---
if user_actual['rol'] == 'admin':
    st.sidebar.markdown("---")
...
if opcion == "1. Subir Cortes Diarios (Excel)":
    ...
    if corte_bloqueado:
        st.warning(f"⚠️ **Atención:** La fecha `{fecha_activa}` ya se encuentra bloqueada/cerrada. No se pueden subir ni sobrescribir archivos a menos que un Administrador la desbloquee desde el menú lateral.")
    else:
        ...
        if up_ventas is not None and up_propinas is not None:
            ...
            if st.button("Guardar corte de Meseros", key="btn_guardar_corte_meseros"):
                guardar_corte_ventas(df_v, df_p, archivo_origen=up_ventas.name, fecha_corte=fecha_activa, usuario_nombre=user_actual['username'])
                st.success(f"¡Corte de meseros y propinas guardado y bloqueado correctamente para el día {fecha_activa}!")
                st.rerun()
            ...
            if st.button("Procesar y Guardar Comisiones del Día", key="btn_guardar_chicas"):
                if len(df_c.columns) >= 5:
                    ...
                    nuevas_detectadas = guardar_corte_chicas(
                        filas_chicas, calcular_comision_chica, archivo_origen=up_chicas.name, fecha_corte=fecha_activa, usuario_nombre=user_actual['username']
                    )
                    st.success(
                        f"¡Corte procesado, guardado y bloqueado para el día {fecha_activa}! Se registraron {len(nuevas_detectadas)} "
...
# --- SECCIÓN 3: CORTE Y NÓMINA FINAL ---
    ...
    if corte_bloqueado:
        st.info("🔒 Esta fecha está bloqueada. Los cambios en los editores de esta sección no se permitirán a menos que se desbloquee el día.")
    ...
    def procesar_grupo_chicas(df_subgrupo, nombre_pestana, key_sufijo):
        ...
        for _, emp in df_subgrupo.iterrows():
            ...
            penalizada_cambiada = st.checkbox(
                ...
                disabled=corte_bloqueado
            )

            if not corte_bloqueado and (penalizada_cambiada != penalizada_actual):
        ...
        disabled_cols = [c for c in cols_mostrar if c not in ["Sueldo Base", "Vales", "Transferencia", "Descuento"]] if not corte_bloqueado else cols_mostrar
        ...
        if not corte_bloqueado and editor_key in st.session_state:
        ...
        col_m1.metric(f"Subtotal Nómina {nombre_pestana}", f"${subtotal:,.2f}")
        col_m2.metric(f"Total Vales {nombre_pestana}", f"${total_vales_grupo:,.2f}")
        col_m3.metric(f"Total Transferencias {nombre_pestana}", f"${total_transf_grupo:,.2f}")
        col_m4.metric(f"Total Descuentos {nombre_pestana}", f"${total_descuento_grupo:,.2f}")
    ...
    def procesar_grupo_general(df_subgrupo, nombre_pestana, key_sufijo):
        ...
        disabled_cols_gen = ["ID", "Nombre", "Puesto", "Propina (%)", "Comisiones", "Total a Pagar"] if corte_bloqueado else ["ID", "Nombre", "Puesto", "Propina (%)", "Comisiones", "Total a Pagar"]
        ...
        df_editado_gen = st.data_editor(
            ...
            disabled=disabled_cols_gen if corte_bloqueado else ["ID", "Nombre", "Puesto", "Propina (%)", "Comisiones", "Total a Pagar"],
            use_container_width=True,
            key=editor_key_gen
        )

        actualizado_gen_flag = False
        if not corte_bloqueado and editor_key_gen in st.session_state:
    ...
    with tab_meseros:
        ...
        ] if not empleados_df.empty else pd.DataFrame()
...
# --- SECCIÓN 4: CIERRE DE CAJA DIARIO (DASHBOARD) ---
    ...
    with col_g1:
        gasto_cocina = st.number_input("Gastos - Cocina ($)", value=g_cocina_val, format="%.2f", key="input_gasto_cocina", disabled=corte_bloqueado)
    with col_g2:
        gasto_compras = st.number_input("Gastos - Compras ($)", value=g_compras_val, format="%.2f", key="input_gasto_compras", disabled=corte_bloqueado)
    with col_g3:
        gasto_vales = st.number_input("Vales / Otros ($)", value=g_vales_val, format="%.2f", key="input_gasto_vales", disabled=corte_bloqueado)

    if not corte_bloqueado:
        if st.button("Guardar Gastos del Día"):
            guardar_gastos_del_dia(gasto_cocina, gasto_compras, gasto_vales, fecha_corte=fecha_activa, usuario_nombre=user_actual['username'])
            st.success(f"¡Gastos del día guardados y bloqueados para la fecha {fecha_activa}!")
            st.rerun()
    else:
        st.info("🔒 Los gastos están bloqueados para esta fecha.")
    ...
    if not ventas_acumuladas.empty:
        efectivo_ventas = float((ventas_acumuladas['efectivo'] + ventas_acumuladas['propina_efectivo']).sum())
        tarjeta_ventas = float((ventas_acumuladas['tarjeta'] + ventas_acumuladas['propina_tarjeta']).sum())
        transferencia_ventas = float((ventas_acumuladas['vales'] + ventas_acumuladas['propina_vales']).sum())
        ventas_por_cobrar = float((ventas_acumuladas['otros'] + ventas_acumuladas.get('propinacredito', 0)).sum())
    ...
    for idx, (titulo, valor) in enumerate(ventas_cards):
        with cols[idx]:
            st.markdown(
                f"""
                <div style="background-color: #141D26; padding: 14px; border-radius: 10px; border: 1px solid #1F2937; text-align: center;">
    ...
    with col_e1:
        st.markdown(
            f"""
            <div style="background-color: #1A2634; padding: 18px; border-radius: 12px; border-left: 5px solid #00E676;">
    ...
    with col_e2:
        st.markdown(
            f"""
            <div style="background-color: #1A2634; padding: 18px; border-radius: 12px; border-left: 5px solid #29B6F6;">
    ...
    if not ventas_acumuladas.empty and not empleados_dashboard_df.empty:
        ...
        resumen_meseros = df_ventas_meseros.groupby('nombre').agg({
            'efectivo': 'sum', 'propina_efectivo': 'sum', 'tarjeta': 'sum', 'propina_tarjeta': 'sum',
            'vales': 'sum', 'propina_vales': 'sum', 'otros': 'sum', 'propinacredito': 'sum'
        }).reset_index()
